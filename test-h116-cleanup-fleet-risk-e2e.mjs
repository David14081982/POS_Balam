...
await page.evaluate(()=>{
  ...
  const preview=()=>({ok:true,system_mode:'preproduction',protocol_version:3,minimum_client_protocol:3,
    ...
    fleet:{summary:window.__h116.blocked?{ready:1,compatible_offline:1,update_on_return:1,attention:1,unsafe_legacy:0,retired:0}:{ready:1,compatible_offline:1,update_on_return:1,attention:0,unsafe_legacy:0,retired:0},
      devices:window.__h116.blocked?devices.concat([{device_id:'pending',display_name:'Caja 2',state:'attention',protocol_version:1,schema_version:20260817014900,data_epoch:11}]):devices},
    blocked_reasons:window.__h116.blocked?[{code:'pending_operation_intersects_cleanup',device_name:'Caja 2',domains:['sales']}]:[],
    executable:!window.__h116.blocked,client_ready:true,ready:!window.__h116.blocked});
...
check('resumen muestra equipos listos',body.includes('1 equipos listos'));
check('apagado compatible no bloquea',body.includes('1 equipo(s) apagado(s) — no bloquea'));
check('equipo viejo se actualiza al volver y no bloquea',body.includes('1 equipo(s) requiere actualización al volver a conectarse — no bloquea'));
check('sin operación pendiente bloqueante',body.includes('0 operación pendiente requiere atención — sí bloquea'));
...
check('códigos internos no aparecen en UI',!body.includes('pending_operation_intersects_cleanup')&&!body.includes('client_schema_incompatible'));
