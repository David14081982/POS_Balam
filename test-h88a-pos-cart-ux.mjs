...
try{
  ...
  for(const width of [320,360,390,430,768,1024,1280,1440]){
    await page.setViewportSize({width,height:width<500?700:800});await page.waitForTimeout(80);
    ...
    if(width<1280)check(s.accessRect.bottom<= (width<500?700:800)+1,`${width}px acceso visible`);
  }
  ...
  await page.keyboard.press('Tab');check(await page.evaluate(()=>!!document.activeElement.closest('[data-testid="pos-cart-surface"]')),'foco permanece en carrito');await page.keyboard.press('Escape');await page.waitForTimeout(80);check(!(await page.getByTestId('pos-cart-surface').count()),'Escape cierra');check(await page.evaluate(()=>document.activeElement?.dataset?.testid==='pos-cart-open'),'foco vuelve al acceso');
