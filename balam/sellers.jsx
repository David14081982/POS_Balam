      ...
      h('div', { className: 'p-10 max-w-container-max mx-auto' }, [
        ...
        h('section', { key: 'hd', className: 'flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6' }, [
          ...
          h('div', { key: 'r', className: 'flex items-center gap-3' }, [
            h('button', { key: 'rep', className: 'flex items-center gap-2 px-6 py-2.5 bg-surface border border-outline-variant text-primary hover:border-primary transition-colors rounded-lg', onClick: () => window.XLSXIO.exportSellers(D.sellers.map(s => ({ nombre: s.nombre, rol: role(s), pct: s.comisionPct || 0, ventas: Math.round(s.ventasMes || 0), meta: Math.round(s.metaMes || 0), avance: metaPct(s), comision: Math.round((s.comisionAcum || 0) * 100) / 100, estado: s.active === false ? 'Inactivo' : 'Activo' }))) }, [h(MS, { key: 'i', name: 'download', size: 18 }), h('span', { key: 's', className: 'text-overline font-bold uppercase tracking-widest' }, 'Reporte mensual')]),
