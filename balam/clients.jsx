      ...
      h('div', { className: 'p-10 max-w-container-max mx-auto' }, [
        ...
        h('div', { key: 'fl', className: 'flex items-center justify-between gap-4 flex-wrap mb-6' }, [
          h('div', { key: 'l', className: 'flex items-center gap-3 flex-wrap' }, [
            h('div', { key: 's', className: 'relative w-80' }, [
              ...
              h('input', { key: 'in', className: 'w-full bg-surface border border-outline-variant rounded-lg pl-10 pr-4 py-2.5 text-body focus:ring-1 focus:ring-primary focus:border-primary transition-all', placeholder: 'Buscar por nombre o teléfono…', value: query, onChange: e => setQuery(e.target.value) }),
            ]),
            h(Segment, { key: 'sg', value: filter, onChange: setFilter, options: [['all', 'Todos'], ['rec', 'Recurrentes'], ['new', 'Nuevos']] }),
          ]),
        ...
        h('div', { key: 'tbl', className: CARD + ' overflow-hidden shadow-e1' }, [
          h('div', { key: 'sc', className: 'overflow-x-auto' }, h('table', { className: 'w-full text-left border-collapse' }, [
            ...
            h('tbody', { key: 'b', className: 'divide-y divide-outline-variant' }, rows.map(c => h('tr', { key: c.id, className: 'hover:bg-surface-container transition-all group cursor-pointer', onClick: () => setDetail(c) }, [
          ...
          h('div', { key: 'pg', className: 'px-6 py-4 border-t border-outline-variant flex items-center justify-between bg-surface-container/30' }, [
            h('span', { key: 'l', className: 'text-overline font-bold text-on-surface-variant uppercase tracking-widest' }, `${rows.length} cliente${rows.length === 1 ? '' : 's'}`),
        ...
        // Drawer
        h(ClientDrawer, { key: 'dr', c: detail, onClose: () => setDetail(null) }),
      ]));
  }
        ...
        c && [
          ...
          h('div', { key: 'c', className: 'flex-grow overflow-y-auto p-8 space-y-8' }, [
            ...
            h('div', { key: 'ac', className: 'pt-2 flex gap-4' }, [
              h('button', { key: 'p', className: 'w-14 h-[52px] rounded-xl border border-outline-variant text-primary hover:bg-surface-container transition-all flex items-center justify-center', onClick: () => toast('Llamando a ' + c.tel), title: 'Llamar' }, h(MS, { name: 'phone', size: 20 })),
              h('button', { key: 'e', className: 'flex-grow bg-primary text-on-primary py-3.5 rounded-xl text-overline font-bold uppercase tracking-widest hover:opacity-90 transition-all shadow-e2 active:scale-95 flex items-center justify-center gap-2', onClick: () => toast('Editar cliente (demo)') }, [h(MS, { key: 'i', name: 'edit', size: 18 }), 'Editar cliente']),
            ]),
          ]),
        ]),
    ]);
