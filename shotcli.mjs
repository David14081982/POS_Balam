...
try{ await p.getByRole('button',{name:'Clientes'}).first().click({timeout:4000}); await p.waitForTimeout(1200);}catch(e){console.log('nav',e.message);}
await p.screenshot({path:'shot-clientes.png'});
try{ await p.locator('tbody tr').first().click({timeout:3000}); await p.waitForTimeout(1000);}catch(e){console.log('row',e.message);}
await p.screenshot({path:'shot-cli-drawer.png'});
