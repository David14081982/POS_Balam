...
BORDADA con hashes de preservación. Verificado: 35/35 TB con barcode, SKU y
existencias iguales; 0 diferencias; online activo.
**Pruebas:** H-175 3/9 → 10/10 (exportación/importación completa sin cambios); H-174 6/6; H-172 7/7; UI 6/6; PWA 2/2; dry-run
...
Cambio acotado a ProductCard; DATA/CONFIG conservadas.
CI y deploy 34784122632 SUCCESS; tres archivos públicos idénticos al commit.
Commit: `f38f50d98e8701560c76a956e556832201fad864`. [Detalle](fixes/h171-circulos-ornamento-pos.md).
[Constancia](fixes/evidence/h171/pos-ornaments/production.json).
...
sincronizar SQL aislado → etiquetas → escanear → recargar con pull. Las dos
referencias se agregan correctamente, pero ambas líneas omiten el SKU comercial.
Rojo 27/31 junto con las 18 comprobaciones SQL de H-138; cuatro fallos de
presentación (dos productos, antes/después de recarga).
**Causa:** el resumen renderiza nombre/talla/color y no `l.p.sku`.
**Alcance:** mostrar el SKU persistido de la referencia exacta. No regenerar
identidades ni cambiar lectura, stock, sincronización o documentos históricos.
**Corrección:** renderizar el SKU persistido de `l.p`, con ajuste de texto al
ancho disponible. Sin sustituir barcode ni generar SKU nuevo.
**Pruebas:** verde ampliado 43/43: altas/edición SQL, PDF decodificado con ZXing,
...
**Pruebas:** rojo 1/4; verde H-143 35/35 local y 35/35 sobre Pages
(POS, Reportes y Apartados); H-85
...
**Estado:** CORREGIDO Y PUBLICADO — VALIDACIÓN FÍSICA PENDIENTE. **Fecha:** 05/09/2026.
...
**Fecha:** 11/09/2026. **Commit:** `06ffb89de43c54a5c64aa1d0e9fe2e75c09fac30`.
...
**Publicación:** commit técnico en `main`, workflow H148 `34647824445`
(intento 2) y H132 `34647824452` correctos. Pages verificado el 11/09/2026
a las 21:15:55 UTC: 10/10 respuestas HTTP 200 idénticas al commit, incluida
la raíz sin parámetros. Evidencia: `docs/fixes/evidence/h158-pages.json`.
**Pendiente:** observar adopción de la actualización en la terminal del usuario.
**Riesgo residual:** una pestaña abierta puede conservar la versión anterior.
La primera ejecución de CI falló en dos aserciones de cola con espera fija;
...
`must_rebootstrap`, `reactivation_requires_sync=true`, protocolo/esquema
compatibles y rechazo de inventario «Este equipo debe actualizar su información
antes de guardar.». No se inspeccionó su cola física ni se probó que fuera fantasma.
**Causas respaldadas:** la guarda SQL impide escribir antes de reconstruir;
el heartbeat exige cola cero para liberar la reactivación; la intención pendiente
impide aplicar el dominio durante la reconstrucción. El Centro suma historia y
...
conservar captura, navegación, permisos y resolución idempotente vigente.
**Criterio:** formulario visible e interactivo durante espera, consulta manual,
recuperación sin remontar; acceso rechazado y sesión distinta siguen protegidos.
**Solución:** franja no bloqueante con consulta manual después de cargar la
sesión; cabecera distingue guardado/confirmación. Arranque y permisos conservados.
