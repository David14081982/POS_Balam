-- H-32: evidencia histórica del descuento por renglón de venta.
--
-- `pos.sale_items.promos` guarda una COPIA congelada de las promociones que produjeron el
-- precio del renglón: [{ id, nombre, tipo, valor }]. Es copia y no referencia, de modo que la
-- venta sigue siendo explicable aunque la promoción se edite o se elimine.
--
-- Un arreglo vacío significa "sin promoción". La AUSENCIA del campo (null) significa
-- "venta anterior a H-32": esas ventas nunca imprimen porcentaje, por diseño.
--
-- No se recalcula, no se rellena y no se reinterpreta ningún renglón existente. Ningún
-- importe cobrado cambia: `precio`, `precio_base`, `precio_original`, `subtotal`, `iva`,
-- `total` y `descuento` conservan exactamente su significado y sus valores.
--
-- Reversión: eliminar la columna y restaurar `pos.commit_sale` a la versión de
-- 20260725001900_pos_transactional_sale.sql. Ningún cliente depende del campo para cobrar.
...
-- Se reemplaza commit_sale únicamente para transportar `promos` hacia pos.sale_items.
-- El resto del cuerpo es idéntico a 20260725001900_pos_transactional_sale.sql.
