-- H-42 (C6) · Atribucion al vendedor y revision de la prenda
--
-- El Contrato del Cambio asigna al vendedor que atiende el cambio la comision
-- del excedente (§7), y la revision de la prenda decide si hay cambio (§5). El
-- modelo calculaba `base_comision` sin poder atribuirla a nadie y no guardaba
-- rastro de quien reviso la mercancia ni de que encontro.
--
-- Tres columnas, dos conceptos, todas aditivas y nullable: los cambios ya
-- registrados —ninguno todavia— conservan su comportamiento.
--
--   exchanges.vendedor_id    quien atiende el cambio y cobra el excedente
--   exchanges.revisado_por   quien reviso la mercancia recibida
--   exchange_items.condicion resultado de esa revision, por prenda
--
-- `commit_exchange` se redefine de forma estrictamente aditiva: el texto se
-- genero a partir de la definicion vigente (20260728005900) aplicando solo dos
-- ediciones (R-DB-03). Firma, orden de validaciones, codigos de error,
-- valoracion, inventario, cobro e idempotencia quedan identicos.
...
comment on column pos.exchanges.vendedor_id is
  'H-42: vendedor que atendio el cambio. Es a quien corresponde la comision de base_comision (Contrato del Cambio, seccion 7).';
comment on column pos.exchanges.revisado_por is
  'H-42: quien reviso la mercancia recibida. La prenda en malas condiciones no se recibe (seccion 5).';
comment on column pos.exchange_items.condicion is
  'H-42: resultado de la revision de esa prenda. Solo aplica al lado devuelto.';
