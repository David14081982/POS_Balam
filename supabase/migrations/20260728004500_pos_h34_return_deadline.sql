-- POS Balam — H-34: plazo de posventa congelado en la venta.
--
-- El plazo para devolver deja de ser una lectura de la configuración vigente y
-- pasa a ser un SNAPSHOT de la venta, igual que el folio (H-33), el desglose
-- financiero (H-03) y la evidencia del descuento (H-32). Sin este contrato, un
-- administrador que activara el límite vencería retroactivamente ventas ya
-- emitidas.
--
-- Contrato de las columnas:
--   return_limit_days IS NULL   → la venta NO vence nunca (sin límite).
--   return_limit_days > 0 y return_expires_at IS NULL
--                               → apartado sin liquidar: el plazo aún no arranca.
--   return_limit_days > 0 y return_expires_at
--                               → vence al terminar ese día del negocio.
--
-- Todas las filas existentes quedan con NULL en ambas columnas —es decir, sin
-- límite—, así que ninguna venta cambia de comportamiento y no se reescribe
-- ningún dato.
--
-- `pos.commit_sale` se redefine de forma ESTRICTAMENTE ADITIVA: misma firma,
-- mismos parámetros y mismo cuerpo que la versión de H-32, más el transporte de
-- las dos columnas nuevas. El texto de la función se generó a partir de la
-- definición vigente para que no exista deriva accidental.
--
-- Reversión: eliminar las dos columnas, sus restricciones y el índice, y
-- restaurar `pos.commit_sale` a la definición de
-- 20260727004000_pos_h32_discount_trace.sql.

begin;

alter table pos.sales
  add column if not exists return_limit_days integer,
  add column if not exists return_expires_at date;

alter table pos.sales
  drop constraint if exists sales_return_limit_days_chk;
alter table pos.sales
  add constraint sales_return_limit_days_chk
  check (return_limit_days is null or return_limit_days > 0);

-- Una fecha de vencimiento sin días congelados sería un plazo sin la política
-- que lo explique: la evidencia está completa o ausente, nunca a medias.
alter table pos.sales
  drop constraint if exists sales_return_deadline_pair_chk;
alter table pos.sales
  add constraint sales_return_deadline_pair_chk
  check (return_expires_at is null or return_limit_days is not null);

-- Respalda el filtro "vencidos" de la pantalla de Devoluciones sin penalizar a
-- las ventas sin límite, que son la mayoría y quedan fuera del índice parcial.
create index if not exists sales_return_expires_at_idx
  on pos.sales (return_expires_at)
  where return_expires_at is not null;
...
begin
  ...
  insert into pos.sales (
    ...
    -- H-34: la AUSENCIA de la clave significa venta sin límite. Un cliente
    -- anterior a esta migración nunca la envía y obtiene NULL, es decir el
    -- mismo resultado que producía la versión previa de esta función.
    nullif(p_sale ->> 'return_limit_days', '')::integer,
    nullif(p_sale ->> 'return_expires_at', '')::date
  )
  on conflict (folio) do update set
    ...
    -- Un reintento que no traiga el plazo NO lo borra: sólo avanza cuando el
    -- cliente envía un valor. Protege la venta ya registrada con plazo.
    return_limit_days = coalesce(excluded.return_limit_days, pos.sales.return_limit_days),
    return_expires_at = coalesce(excluded.return_expires_at, pos.sales.return_expires_at),
    updated_at = now();

  delete from pos.sale_items where folio = v_folio;
  insert into pos.sale_items (
    folio, product_id, sku, nombre, talla, qty, precio, precio_base, precio_original, promos
  )
  select v_folio, x.product_id, x.sku, x.nombre, x.talla, x.qty,
         x.precio, x.precio_base, x.precio_original, x.promos
