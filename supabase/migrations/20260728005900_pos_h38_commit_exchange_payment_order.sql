-- H-38 (C5) · Corrección: el cobro se valida antes de escribir
--
-- Defecto detectado por la verificación 20260728006000 al aplicarse sobre la
-- base real:
--
--     ERROR: H-38: acepto un cobro que no cuadra con la diferencia:
--            {"ok": false, "error": "exchange_id_conflict"}
--
-- Causa raíz: en 20260728005700 las salidas `payment_required` y
-- `payment_mismatch` ocurrían DESPUÉS de insertar cabecera, renglones y
-- movimientos. Un `return` de plpgsql no aborta la transacción, de modo que un
-- cobro mal formado dejaba el documento escrito a medias y el siguiente intento
-- chocaba con `exchange_id_conflict`. Es exactamente la escritura parcial que
-- H-04 existe para impedir.
--
-- 20260728005700 ya está registrada, así que no se reescribe (R-DB-01): se
-- corrige hacia adelante. El texto se generó a partir de la definición vigente
-- aplicando sólo dos ediciones (R-DB-03): mover la validación del cobro delante
-- de la primera escritura, y dejar el paso 10 como una inserción simple.
-- Ninguna otra línea cambia: firma, orden del resto de validaciones, códigos de
-- error, valoración, inventario y respuesta son idénticos.
--
-- La verificación se renumeró de 005800 a 006000 porque debe correr al final
